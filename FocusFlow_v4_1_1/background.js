...
async function handleTabFocus(tabId) {
  try {
    ...
    if (isBlocked(domain)) {
      chrome.tabs.update(tabId, { url: chrome.runtime.getURL("blocked.html") + "?site=" + encodeURIComponent(domain) });
      await stopTracking(); return;
    }
...
const DEEP_WORK_ALARM = "deepWorkAlarm";
const SYNC_ALARM = "focusflowSyncAlarm";

chrome.alarms.onAlarm.addListener(async (alarm) => {
  if (alarm.name === TICK_ALARM) { await onTick(); return; }
  if (alarm.name === POMO_ALARM) { await handlePomoAlarm(); return; }
  if (alarm.name === DEEP_WORK_ALARM) { await finishDeepWork(true); return; }
  if (alarm.name === SYNC_ALARM) { try { await SyncManager.runSync(); } catch {} }
});

// Keep the sync alarm in step with the toggle in Settings > Website Sync,
// even while the popup (not the service worker) is what changed it.
chrome.storage.onChanged.addListener(async (changes, area) => {
  if (area !== "local" || !changes.syncConfig) return;
  const config = changes.syncConfig.newValue || {};
  if (config.autoSync && config.url) {
    const existing = await chrome.alarms.get(SYNC_ALARM);
    if (!existing) chrome.alarms.create(SYNC_ALARM, { periodInMinutes: 30 });
  } else {
    await chrome.alarms.clear(SYNC_ALARM);
  }
});

async function getDeepWorkState() {
  const r = await chrome.storage.local.get("deepWorkState");
  return r.deepWorkState || { running: false, endTime: null, goal: "", durationMin: 0 };
}

async function finishDeepWork(completed = true) {
  const state = await getDeepWorkState();
  if (!state.running) return;
  await chrome.alarms.clear(DEEP_WORK_ALARM);
  const restore = state.previousBlockedSites || [];
  BLOCKED_SITES = restore;
  await chrome.storage.local.set({
    blockedSites: restore,
    deepWorkState: { running: false, endTime: null, goal: "", durationMin: 0 }
  });
  if (completed) {
    const today = getTodayKey();
    const r = await chrome.storage.local.get("deepWorkHistory");
    const history = r.deepWorkHistory || [];
    history.push({ date: today, durationMin: state.durationMin || 0, goal: state.goal || "", completedAt: new Date().toISOString() });
    await chrome.storage.local.set({ deepWorkHistory: history.slice(-200) });
    try {
      await chrome.notifications.create("deep-work-complete-" + Date.now(), {
        type: "basic", iconUrl: chrome.runtime.getURL("icon128.png"),
        title: "Deep Work complete",
        message: state.goal ? `Nice work: ${state.goal}` : "Your deep-work session is complete. Take a short break."
      });
    } catch {}
  }
  try {
    const tabs = await chrome.tabs.query({});
    for (const tab of tabs) if (tab.id && tab.url && tab.url.startsWith(chrome.runtime.getURL("blocked.html"))) {
      chrome.tabs.update(tab.id, { url: "chrome://newtab/" });
    }
  } catch {}
}

async function startDeepWork(durationMin, goal = "") {
  const current = await getDeepWorkState();
  if (current.running) return current;
  const r = await chrome.storage.local.get("blockedSites");
  const previous = r.blockedSites || [];
  const merged = [...new Set([...previous, ...DEFAULT_BLOCKED])];
  const endTime = Date.now() + durationMin * 60 * 1000;
  const state = { running: true, endTime, goal, durationMin, previousBlockedSites: previous };
  BLOCKED_SITES = merged;
  await chrome.storage.local.set({ blockedSites: merged, deepWorkState: state });
  await chrome.alarms.create(DEEP_WORK_ALARM, { when: endTime });
  return state;
}

async function init() {
  const r = await chrome.storage.local.get(["activeTracking", "blockedSites"]);
  if (r.activeTracking) {
  ...
  const dw = await getDeepWorkState();
  if (dw.running && dw.endTime && dw.endTime > Date.now()) {
    const exDw = await chrome.alarms.get(DEEP_WORK_ALARM);
    if (!exDw) chrome.alarms.create(DEEP_WORK_ALARM, { when: dw.endTime });
    BLOCKED_SITES = [...new Set([...(r.blockedSites || BLOCKED_SITES), ...DEFAULT_BLOCKED])];
  } else if (dw.running) {
    await finishDeepWork(true);
  }
  ...
  const syncConfig = await SyncManager.getSyncConfig();
  if (syncConfig.autoSync && syncConfig.url) {
    const exSync = await chrome.alarms.get(SYNC_ALARM);
    if (!exSync) chrome.alarms.create(SYNC_ALARM, { periodInMinutes: 30 });
  }
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === "DEEP_WORK_GET_STATE") {
    getDeepWorkState().then(sendResponse); return true;
  }

  if (msg.type === "DEEP_WORK_START") {
    (async () => {
      const durationMin = Math.max(1, Math.min(240, Number(msg.durationMin) || 25));
      const state = await startDeepWork(durationMin, String(msg.goal || "").slice(0, 120));
      sendResponse(state);
    })(); return true;
  }

  if (msg.type === "DEEP_WORK_STOP") {
    (async () => { await finishDeepWork(Boolean(msg.completed)); sendResponse(await getDeepWorkState()); })(); return true;
  }
